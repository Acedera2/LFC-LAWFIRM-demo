...
export default function ClientDashboard() {
  ...
  useEffect(() => {
    ...
    api
      .get("/lawyers")
      .then((response) => {
        const data = unwrap(response).lawyers || [];
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((response) => {
        const data = unwrap(response).appointments || [];
  ...
  const lawyerOptions = lawyers.length
    ? lawyers.map((lawyer) => ({
        ...
        specialty: lawyer.specialization || "Legal specialty"
      }))
    : [{ id: "", name: "No lawyers available", specialty: "" }];
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      setFiles([]);
      setScan(null);
      setAppointments((current) => [appointment, ...current].slice(0, 5));
    } catch (error) {
      toast.error(error.response?.data?.message || "Could not submit appointment inquiry");
    }
  };

  const requestsOpen = appointments.filter((item) => item.status !== "COMPLETED").length || 3;
  ...
  return (
    <div className="grid gap-6">
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <StatCard icon={CalendarPlus} label="Open requests" value={`${requestsOpen}`} trend="Live client inquiries" />
        <StatCard icon={Clock3} label="Next consult" value={nextLabel} trend="Staff will confirm assignment" tone="brass" />
        <StatCard icon={FileUp} label="Documents" value="Managed securely" trend="Upload with inquiry" tone="blue" />
        <StatCard icon={History} label="Recent activity" value="Live updates" trend="Request history available" tone="jade" />
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1fr_0.95fr]">
        <section className="rounded-3xl border border-ink-100 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-white/5">
          ...
          <form onSubmit={submit} className="mt-6 grid gap-4 md:grid-cols-2">
            ...
            <label className="grid gap-2 text-sm font-bold text-ink-700 dark:text-white">
              Consultation mode
              <select value={form.locationMode} onChange={(event) => setForm((current) => ({ ...current, locationMode: event.target.value }))} className="focus-ring rounded-3xl border border-ink-100 px-3 py-3 dark:border-white/10 dark:bg-ink-950">
        ...
        <section className="rounded-3xl border border-ink-100 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-white/5">
          ...
          <div className="mt-5 grid gap-4">
            {loadingAppointments ? <LoadingSkeleton rows={3} /> : appointments.length === 0 ? <EmptyState title="No recent inquiries" message="New appointment requests will show here after you submit them." /> : appointments.map((appointment) => <AppointmentCard key={appointment.id} appointment={appointment} />)}
          </div>
        </section>
      </div>
    </div>
